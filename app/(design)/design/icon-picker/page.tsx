 ...
 *
 * Picker generalizzato per cambiare l'icona di una tappa dal dettaglio.
 * Vive in un design a parte (estratto da timeline-readability it.11b)
 * perché è un componente trasversale.
 *
 * Vincolo chiave: il dominio delle icone è SEMPRE quello delle categorie
 * di ExploreToolbar — `CategoryIconPicker` deriva tutto da
 * EXPLORE_CATEGORY_TREE via useExploreCategories(), quindi una categoria
 * nuova compare automaticamente in toolbar E nel picker.
 *
 * Demo interattiva: tap sul badge icona apre/chiude il popover, tap su
 * un'icona seleziona e chiude.
 */
    ...
    <div className="mx-auto max-w-[900px] px-6 py-10">
      <header className="mb-8">
        ...
        <p className="max-w-[680px] text-meta leading-relaxed text-ink-soft">
          ...
          all&apos;albero e la trovi qui, senza toccare il picker. Label
          i18n come tooltip/aria.
        </p>
      </header>
      ...
      {/* Demo 3 — IconPicker puro con un dominio custom, a prova che
          la generalizzazione regge (qui: solo la macro "mangia"). */}
      <section className="mb-12">
        <h2 className="mb-1 text-[17px] font-semibold text-ink">
          Dominio custom (IconPicker puro)
        </h2>
        <p className="mb-4 text-mini text-ink-soft">
          Il presentazionale accetta qualsiasi <code className="rounded bg-surface-soft px-1 text-[12px]">groups</code>:
      ...
      <section className="rounded-md border border-border bg-surface p-4">
        ...
        <div className="flex flex-col gap-1.5 text-mini leading-relaxed text-ink-soft [&_code]:rounded [&_code]:bg-surface-soft [&_code]:px-1 [&_code]:text-[11px] [&_code]:text-ink">
          <p>
            ...
            click-out; focus trap + frecce per a11y. Persistenza: la
            selezione scrive l&apos;id categoria su <code>activity.icon</code>{" "}
            (entity-level, <code>ActivityService.updateEntity</code>) — oggi
            il campo contiene chiavi <code>STOP_ICONS</code> (&quot;shop&quot;,
            &quot;coffee&quot;…): serve mappa di compatibilità in{" "}
            <code>getStopIcon</code> (vecchie chiavi → icona) o migrazione
            una tantum verso gli id categoria. Accommodation: NON usa questo
            picker — il tipo struttura resta lo StayTypePicker
            (timeline-readability it.11).
          </p>
        </div>
      </section>
    </div>
